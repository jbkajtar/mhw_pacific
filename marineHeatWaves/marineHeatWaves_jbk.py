...
def detect(t, temp, climatologyPeriod=[None,None], pctile=90, windowHalfWidth=5, smoothPercentile=True, smoothPercentileWidth=31, minDuration=5, joinAcrossGaps=True, maxGap=2, maxPadLength=False, coldSpells=False, alternateClimatology=False, Ly=False):
    ...
    # new vars (added by JBK)
    mhw['time_peak_norm'] = [] # datetime format
    mhw['date_peak_norm'] = [] # datetime format
    mhw['index_peak_norm'] = []
    mhw['intensity_max_norm'] = [] # normalised intensity, multiple of threshold minus climatology
    ...
    for ev in range(mhw['n_events']):
        ...
        # Find peak normalised intensity (added by JBK)
        mhw_norm = (temp_mhw - seas_mhw) / (thresh_mhw - seas_mhw)
        tt_peak_norm = np.argmax(mhw_norm)
        mhw['time_peak_norm'].append(mhw['time_start'][ev] + tt_peak_norm)
        mhw['date_peak_norm'].append(date.fromordinal(mhw['time_start'][ev] + tt_peak_norm))
        mhw['index_peak_norm'].append(tt_start + tt_peak_norm)
        mhw['intensity_max_norm'].append(mhw_norm[tt_peak_norm])
        #print(np.max(mhw_relThreshNormClim))
        # Fix categories
        ...
        if tt_end < T-1:
            ...
            if tt_peak == tt_end-tt_start: # Peak is also at end of time series, assume decline time = 1 day
...
def blockAverage(t, mhw, clim=None, blockLength=1, removeMissing=False, temp=None, coldSpells=False):
    '''
    ...
    if sw_temp:
        ...
        mhwBlock['temp_anom_mean'] = np.zeros(nBlocks)
        mhwBlock['temp_anom_max'] = np.zeros(nBlocks)
        mhwBlock['temp_anom_min'] = np.zeros(nBlocks)
        mhwBlock['temp_anom_cum'] = np.zeros(nBlocks)
    ...
    for i in range(mhw['n_events']):
        ...
        if coldSpells:
             mhwBlock['intensity_max_max'][iBlock] = np.min([mhwBlock['intensity_max_max'][iBlock], mhw['intensity_max'][i]])
        else:
    ...
    if sw_temp:
        for i in range(int(nBlocks)):
            ...
            temp_anom = temp - clim['seas']
            mhwBlock['temp_anom_mean'][i] = np.nanmean((year >= mhwBlock['years_start'][i]) * (year <= mhwBlock['years_end'][i]) * mhwIndex * temp_anom)
            #mhwBlock['temp_anom_max'][i] = np.nanmax(temp_anom[tt])
            mhwBlock['temp_anom_max'][i] = np.nanmax((year >= mhwBlock['years_start'][i]) * (year <= mhwBlock['years_end'][i]) * mhwIndex * temp_anom)
            mhwBlock['temp_anom_min'][i] = np.nanmin((year >= mhwBlock['years_start'][i]) * (year <= mhwBlock['years_end'][i]) * mhwIndex * temp_anom)
            mhwBlock['temp_anom_cum'][i] = np.nansum((year >= mhwBlock['years_start'][i]) * (year <= mhwBlock['years_end'][i]) * mhwIndex * temp_anom)
